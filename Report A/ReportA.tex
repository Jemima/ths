...
Many \acp{FPGA} marketed specifically at space based applications are, in addition to requiring custom hardware, only latchup\fxnote{explain?} immune or only include inbuilt \ac{TMR} on registers, leaving them still vulnerable to \acp{SEU} \cite{FPGAReview}.
...
\begin{figure}
    \begin{center}
        \subfloat[CLB Architecture]{
            \includegraphics[clip,trim=8cm 4cm 8cm 8cm]{images/CLB.pdf}
            \label{Arch}
        }
        \subfloat[Wilton Switch Block\cite{WiltonSwitch}]{
...
\section{Design}
See Section \ref{Algorithm}\fxnote{Recap?}
...
%Things to check with Diessel:
%How much detail to go into about Wilton switches
%Random factor in placing/routing?
%How to show results, how many results to show?
%critical path length? How to describe it
%Explain latchup?
