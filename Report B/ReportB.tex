...
% otherwise the current date is printed
      ...
      \newacronym{ABC}{ABC}{Good Question}
      ...
      \newglossaryentry{cut}{name=cut,
         description={Cut a signal},
      first=\textit{cut}
      }
\begin{document}
      \fxerror{Latch vs Flip-Flop. Be consistent, probably prefer. Actual FPGA hardware seems to be a general purpose register than can behave as either. Probably FF that can be a LUT?}
   \pagenumbering{roman}
   \maketitle
   ...
   \pagenumbering{arabic} \section{Overview} Space plays an increasingly
   ...
   \fxfatal{rewrite} The remainder of this chapter provides an overview of
   ...
   \glsunset{VHDL} \subsection{\glspl{FPGA}} \glspl{FPGA} are popular devices
   ...
   \fxnote{TODO: Use own image. Wilton lecture notes have no license/copyright
      ...
      Plus, doesn't look that good.} \begin{figure} \begin{center}
   ...
   \fxnote{Check grammar in this next section.} There are four main
   ...
   \begin{itemize}
      ...
      \end{table} \fxwarning{Explain columns} Additionally, for \gls{SRAM}
...
\glsunset{TMR} \section{\gls{TMR}}\label{secTMR} Triple Modular Redundancy is a
...
\glsunset{TMR} \subsection{\gls{TMR} Implementations} This thesis builds on the
...
latchup\fxnote{explain?} immune or only include inbuilt \gls{TMR} on registers,
...
\ref{?}\fxfatal{Reference} was chosen.

   \subsection{Our Algorithm}\label{Algorithm} \begin{figure} \begin{center}
...
\glsunset{CAD} \section{\gls{CAD} Flow} \glspl{FPGA} are typically programmed
...
subprocesses\fxnote{hyphenate?} as illustrated in Figure
\ref{CADFlow}\cite{VPRBook,VPRManual,FPGAArch}.  \begin{enumerate}
   ...
   be leaving them at their default.  Section \fxfatal{Reference} discusses the
   ...
   different algorithms:\fxnote{Awkward phrasing, fix} breadth\_first, which
   ...
   algorithm.\fxnote{TODO: Reason?} There are a number of options setting
...
:\begin{enumerate}
         ...
         Algorithm\fxfatal{Expand or cut?} \subsection{Assessment of
 ...
 Partitioning Algorithm, etc
   ...
   \subsection{Blif} Contains helper functions to read in a \gls{BLIF} and
   represent it as a \gls{DFG}.  The circuit itself is represented as a Model
   within Blif.  \begin{table} \begin{tabularx}{\linewidth}{llX} \toprule Field
         Name & Type & Description\\ \midrule masterOutputs &
         \textbf{List(String)} & List of outputs for the original file \\
         masterInputs & \textbf{List(String)} & List of inputs for the original
         file \\ main & \textbf{Model} & The main circuit in the \gls{BLIF}
         file\\ \bottomrule \end{tabularx} \caption{Fields in Blif object}
   \end{table}

   \subsection{Model} Represents the circuit as a \gls{DFG}, with a list of
   ...
   \begin{table} \begin{tabularx}{\linewidth}{lXX} \toprule Field Name & Type &
         Description\\ \midrule name & \textbf{ String} & Name of the circuit
         \\ signals & \textbf{Map(String $\to$ Signal, DEFAULT: NULL)} & Map
         from signal name to Signal object \\ outputs & \textbf{List(Signal)} &
         List of output Signal objects \\ inputs & \textbf{List(Signal)} & List
         of input Signal objects \\ nodes & \textbf{List(BlifNode)} & List of
         all nodes in a circuit \\ numLatches & \textbf{ Integer} & Number of
         latches within a circuit \\ numLUTs & \textbf{ Integer} & Number of
         LUTs within a circuit \\ \bottomrule \end{tabularx} \caption{Fields in
         Model object} \end{table}

   \subsection{BlifNode} Contains the names of the input and output Signals, as
   well as the properties of the node (type, etc).  Does not contain direct
   references to Signals, merely their names.  \begin{table}
      \begin{tabularx}{\linewidth}{llX} \toprule Field Name & Type &
         Description\\ \midrule output & \textbf{ String} & Name of output
         signal \\ clock & \textbf{ String} & Name of clock signal \\ inputs &
         \textbf{List(string)} & List of input signal names \\ cost & \textbf{
            Integer} & How many clock cycles this node contributes to the
         critical path. 0 for \glspl{LUT} and 1 for latches. \\ type & \textbf{
            String} & Type of node, ``latch'' or ``names'' (\gls{LUT}) \\
         ...
         initial latch state\\ \bottomrule \end{tabularx} \caption{Fields in
         BlifNode object} \end{table}

   \subsection{Signal} Contains references to the signal source, and a list of
   its sinks. Also stores the signal name.  \begin{table}
      \begin{tabularx}{\linewidth}{llX} \toprule Field Name & Type &
         Description\\ \midrule name & \textbf{ String} & Name of the signal \\
         source & \textbf{BlifNode} & Pointer to source node which drives this
         signal \\ sinks & \textbf{List(BlifNode)} & List of pointers to this
         node's sinks.\\ \bottomrule \end{tabularx} \caption{Fields in Signal
         object} \end{table}

   \subsection{\gls{DFG} Traversal} BlifNodes represent nodes in the \gls{DFG}
   ...
   signal$\rightarrow$ node. However, BlifNodes do not store a pointer to the
   ...
   looked up in the partition by name. To this end, Model contains a signal
   field which is a map from signal name to Signal.

   Thus, an example which recursivelytraverses from a node to its children
   would be: \begin{algorithm} \caption{Example Traversal}\label{algExTrav}
      \begin{algorithmic}[1] \Procedure{ExampleTraversal}{$startNode$,
            $partition$} \State $outputName \gets startNode->output$
         \Comment{Get the name of our output signal} \State $outputSignal \gets
         partition.signals[outputName]$ \Comment{From a Signal name, get the
            Signal object} \ForAll{$childNode \in outputSignal.sinks$}
         \Comment{Retrieve the sinks of a signal, which are also the immediate
            childen of our start node} \State Reached Child, recursively
         repeating from child.  \State ExampleTraversal$(childNode, partition)$
         \Comment{Recursively visit the child} \EndFor \EndProcedure
   ...
   \begin{algorithm} \begin{center} \begin{tabular}{lll} \toprule Variable &
         ...
         \Procedure{Main}{$input$, $targetRecoveryTime$} \State $files \gets
         \mbox{Partition}(input, targetRecoveryTime)$ \ForAll {$file \in
            files$} \State $file \gets \mbox{Triplicate}(file)$ \EndFor \State
   ...
   \end{table} \begin{algorithm} \caption{Partition}\label{partition}
      \begin{algorithmic}[1] \Procedure{Partition}{$file$} \State $blif \gets$
         new Blif(file) \Comment{Read in $file$} \State $circuit \gets
         blif.main$ \Comment{The actual circuit within the blif file} \State
         $partition \gets$ new Model \Comment{Empty Circuit} \State $queue
         \gets$ new Queue \Comment{Empty Queue} \State $visited \gets$ new
         Map(BlifNode $\to$ bool, DEFAULT: false)

         \ForAll{$signal \in circuit.outputs$} \State
         $queue.\mbox{Enqueue}(signal.source)$ \EndFor

         \While{$queue.size > 0$} \State $node \gets queue.\mbox{Dequeue()}$
         \If{$visited[node] = $ true} \State continue \Comment{Handle each node
            once and only once} \EndIf \State $visited[node] \gets $ true
         \State $partition.\mbox{AddNode}(node)$
         \If{$partition.\mbox{RecoveryTime}() > targetRecoveryTime$} \State
         ...
         numPartitions+1$ \State $partition \gets$ new Model \Comment{Empty
            Circuit} \State $partition.\mbox{AddNode}(node)$ \EndIf
         \ForAll{$signalName \in node.inputs$} \State $signal \gets
         model.signals[signalName]$ \State $queue.\mbox{Enqueue}(signal)$
         \EndFor \EndWhile \If{$partition.size > 0$} \State
   ...
   transforming from the \gls{BLIF} format described in \fxfatal{TODO:
      Reference} to the one represented by \fxfatal{TODO: Reference}.  Lines
   ...
   \begin{algorithm} \begin{center} \begin{tabularx}{\linewidth}{llX} \toprule
      ...
      \caption{MakeIOList}\label{makeiolist} \begin{algorithmic}[1]
         \Procedure{MakeIOList}{$partition, originalCircuit$} \ForAll{$signal
            ...
            this signal has no driver} \State{$partition.inputs.Add(signal)$}
         ...
         \Comment{If the signal has more sinks in the original circuit than it
            does in this partition} \State $partition.outputs.Add(signal)$
         \EndIf \EndFor \EndProcedure \end{algorithmic} \end{algorithm}
   
   \begin{figure} \begin{center}
         \includegraphics[width=\linewidth]{images/MakeIOList.pdf}
         \caption{MakeIOList} \label{imMakeiolist} \end{center} \end{figure} We
   iterate through every signal in our partition. For each one we check if we
   have a source (line 4), if not it must be a primary input. Similarly, on
   ...
   error recovery time.  \begin{algorithm} \begin{center}
      ...
      \begin{algorithmic}[1] \Procedure{RecoveryTime}{$partition$} \State
   ...
   an acyclic graph with every node reachable.  \begin{algorithm}
      ...
      \caption{AddNode}\label{addnode} \begin{algorithmic}[1]
         \Procedure{AddNode}{$partition, node$} \State $nodes.insert(node)$
         \ForAll{$name \in node.inputs$} \If{IsRenamed($signalName$)} \State
         ...
         \State Replace$(node.inputs, signalName, newName)$ \Comment{Replace
            the original name with what it was renamed to} \State $signalName
         ...
         \State $inCost \gets 0$ \ForAll{$signalName \in node.inputs$} \State
         $signal \gets partition.signals[signalName]$ \State $source \gets
         signal.source$ \If{$partition.costs[source] > inCost$} \State $inCost
   ...
   \newpage \subsection{UpdateCostsAndBreakCycles} Recursively traverse our
   ...
   \begin{algorithm} \begin{center} \begin{tabularx}{\linewidth}{llX} \toprule
            Variable & Type & Description\\ \midrule $partition$ &\textbf{
               Model* } &  Model* containing DFG representing partition to add
            node to\\ $root$ &\textbf{  BlifNode* } &  Newly added node\\
            ...
            of signal name to Signal* \\ $parent.output$ &\textbf{ String } &
            Name of the signal the parent nodes drives i.e. the signal we
            reached this node from\\ $signal$ & \textbf{ Signal* } &  Signal we
            reached this node from\\ $node.cost$ &\textbf{ Integer } &  1 for
            latches, 0 for LUTs \\ $costs$ &\textbf{  Map(BlifNode* $\to$
               Integer) } &  Map of the cost to reach each node \\ $node$
            &\textbf{  BlifNode* } &  \\ $signal.sinks$ &\textbf{
               List(BlifNode*) } &  List of sinks for a Signal* \\ $cost$
            &\textbf{ Integer } &  Number of critical path steps to reach node,
            including the node itself \\ \bottomrule \end{tabularx}
      \end{center} \caption{UpdateCostsAndBreakCycles}\label{updatecosts}
      \begin{algorithmic}[1] \Procedure{UpdateCostsAndBreakCycles}{$partition,
            root, parent, node, costToReach, explored$} \If{$explored[node] =
            true $ and $costs[node] \ge costToReach$} \Comment{Already expanded
            this path, and we haven't found a more expensive route to it. No
            point continuing down it} \State return \EndIf \If{$parent \neq
            NULL $ and $ node = root$} \Comment{We have a cycle, as all newly
            created cycles must go through the new node, and the new node
            should only ever be reached once at the start without cycles}
         \State $signal \gets partition.signals[parent.output]$ \Comment{The
            signal edge we came in on} \State CutSignal$(partition, signal)$
         \State return \EndIf \State $cost \gets costToReach+node.cost$
         \If{$cost > costs[node]$} \State $costs[node] = cost$ \Else \State
         $cost = costs[node]$ \EndIf \ForAll{$child \in
            partition.signals[node.output].sinks$} \State
   ...
   \begin{enumerate}
      ...
      \ref{alfCutsignal}\fxfatal{Need to create section}, recurse back a step,
   ...
   \gls{ABC}\cite{abc}\fxwarning{link} to flatten it, and postprocess if
   necessary.  \begin{algorithm} \begin{center}
      ...
      \caption{Flatten}\label{Flatten} \begin{algorithmic}[1]
         \Procedure{Flatten}{$file$} \Comment{Flattening is currently performed
            by \gls{ABC}, called with parameters:} \State ./abc -o output -c
         echo file \Comment{Due to bug in \gls{ABC}, clock information is
            stripped from latches, so we then call grep and sed to fix the
            output file} \State $latch \gets $split(grep -m 1 '\.latch' file)
         \If{$latch$} \State sed -ri
   ...
   benchmark circuits.  \fxfatal{Add in table of results}
   
   \section{Correctness} A threefold approach to verifying the correctness of
   ...
   \section{Design Choices} As much as possible, we would like our
   ...
   compararing the two options can be found at \fxfatal{Reference and add}.
   ...
   \gls{VTR} project \fxwarning{Reference} and described in table
   ...
   \end{table}\fxfatal{Correc to use actual results} \subsection{Target
   ...
   each \gls{CLB} has 33 inputs and 10 outputs.  \fxerror{Compare 6-LUT vs
      4-LUT architecture, and how they compare to benchmark circuits}
   ...
   \end{eqnarray*}\fxerror{Clarify table} In Table \ref{tabSanityPartitions}
   Outputs is the number of feedforward edges (signals going to other
   partitions) $+$ the number of feedback edges (cut cycles).  \fxerror{Explain
      estimating clock period and number of partitions} Some other observations
   ...
   \section{Stochastic Nature of Placement} As \gls{VPR}'s placer uses
   ...
   run with exactly the same set of parameters had a $140\%$ slowdown.  The
   number of runs was arbitrarily set at 10 as a compromise between accuracy
   and running time. 10 runs for one set of parameters takes around half a day.
   \begin{table} \begin{tabular}{lrrrr} \toprule Name & NumPartitions &
         Frequency Base (ns) & Frequency TMR (ns) & Slowdown Factor\\ \midrule
         s38584.1.blif &	1 & 3.22 & 4.61 & 1.43\\ s38584.1.blif &	1 & 2.06 &
         4.94 & 2.40\\ \bottomrule \end{tabular} \caption{Comparison of
         slowdown factors between runs with same input
         parameters}\label{tabStochastic} \end{table}

   \section{Area} As expected, area usage is slightly greater than tripled,
   ...
   \section{Operating Frequency} In general, the more partitions the slower the
   ...
   length upwards. Average slowdown is around the \fxfatal{Value} mark, though
   ...
   \section{Running Time} Seconds for the entire partitioning process, minutes
   for VPR on the first file, and around 4 times longer for VPR on the TMR
   file, up to one hour. \fxfatal{Flesh out section}

   \section{Recovery Time}
   
   \section{DFS vs BFS} See appendix. \fxfatal{flesh out}


   \chapter{Limitations and Future Work} Our algorithm implementation is still
   ...
   better solutions.  And lastly, the partitioner itself makes no attempt to
   ...
   \chapter{Conclusion} This thesis was focussed on developing a new TMR
   ...
   partitioning method described by \fxfatal{reference} and implemented by this
   ...
   calculate the relationships discussed in this thesis.
      ...
      \begin{table} \footnotesize \begin{adjustwidth}{-1cm}{-1.2cm}
            \begin{tabularx}{1.1\textwidth}{XXXXXX} \toprule File & Number of
               Partitions &  Number of BLEs (original) &  Increase in BLE
               Number &  Clock Period (original) (ns) &  Clock Slowdown
               Factor\\ \midrule clma.blif & 4 & 8365 & 3.08 & 9.47 & 1.29\\
               s38584.1.blif & 3 & 6177 & 3.27 & 4.92 & 1.74\\ s38417.blif & 3
               & 6042 & 3.27 & 6.36 & 1.26\\ ex1010.blif & 2 & 4598 & 3.27 &
               6.69 & 1.44\\ pdc.blif & 2 & 4575 & 3.12 & 6.33 & 1.38\\
               spla.blif & 2 & 3690 & 3.12 & 5.83 & 1.47\\ elliptic.blif & 2 &
               3602 & 3.38 & 7.6 & 1.21\\ frisc.blif & 2 & 3539 & 3.35 & 10.91
               & 1.31\\ s298.blif & 1 & 1930 & 3.01 & 8.5 & 1.36\\ apex2.blif &
               1 & 1878 & 3 & 4.91 & 1.24\\ seq.blif & 1 & 1750 & 3.02 & 4.36 &
               1.23\\ bigkey.blif & 1 & 1699 & 3.21 & 2.3 & 1.19\\ des.blif & 1
               & 1591 & 3.15 & 4.15 & 1.05\\ alu4.blif & 1 & 1522 & 3.01 & 4.32
               & 1.26\\ diffeq.blif & 1 & 1494 & 3.28 & 6.4 & 1.17\\
               misex3.blif & 1 & 1397 & 3.01 & 4.72 & 1.14\\ dsip.blif & 1 &
               1362 & 3.17 & 2.19 & 1.29\\ apex4.blif & 1 & 1262 & 3.02 & 4.56
               & 1.28\\ ex5p.blif & 1 & 1064 & 3.06 & 4.63 & 1.24\\ tseng.blif
               & 1 & 1046 & 3.49 & 5.82 & 1.23\\ Mean &         &           &
               &          & 1.29\\ \bottomrule \end{tabularx} \caption{Results
               for target recovery time $2.5\times10^{-4}$s}
            \label{Results2.5e-4} \end{adjustwidth} \end{table}
...
\label{Results7.5e-5DFS} \end{adjustwidth} \end{table}
   ...
   \fxfatal{TODO: Consistency between flip-flop vs latch, when using BLE}

\end{document} \begin{table} \footnotesize \begin{adjustwidth}{-1cm}{-1.2cm}
      \begin{tabularx}{1.1\textwidth}{llllp{1.5cm}p{2.2cm}p{1.5cm}X} \toprule
         Name & Num. Latches & Num. LUTs & FPGA Width & Channel Width & Av. No.
         Wire\newline Segments & Frequency (MHz) & CPU Time (s)\\\midrule alu4
         & 0 & 1522 & 15 & 200 & 4.76391 &225 & 34.566\\ alu4TMR & 0 & 4574 &
         25 & 200 & 5.29148 &188 & 132.41\\\midrule apex2 & 0 & 1878 & 16 & 200
         & 5.5124 &216 & 47.45\\ apex2TMR & 0 & 5637 & 29 & 200 & 6.21459 &172
         & 197.825\\\midrule apex4 & 0 & 1262 & 14 & 200 & 5.0436 &249 &
         31.272\\ apex4TMR & 0 & 3805 & 23 & 200 & 5.33318 &204 &
         119.002\\\midrule bigkey & 224 & 1699 & 16 & 200 & 3.70506 &427 &
         56.61\\ bigkeyTMR & 672 & 5294 & 27 & 200 & 4.93838 &377 &
         193.388\\\midrule clma & 33 & 8365 & 34 & 200 & 5.62286 &115 &
         379.801\\ clmaTMR & 99 & 25177 & 59 & 200 & 6.2762 &103 &
         2146.4\\\midrule des & 0 & 1591 & 16 & 200 & 4.07107 &262 & 98.709\\
         desTMR & 0 & 5018 & 27 & 200 & 3.9862 &229 & 263.883\\\midrule diffeq
         & 377 & 1494 & 15 & 200 & 3.0611 &157 & 60.103\\ diffeqTMR & 1131 &
         4521 & 25 & 200 & 3.44041 &156 & 204.691\\\midrule dsip & 224 & 1362 &
         14 & 200 & 3.823 &433 & 60.372\\ dsipTMR & 672 & 4283 & 24 & 200 &
         4.74885 &377 & 177.405\\\midrule elliptic & 1122 & 3602 & 23 & 200 &
         4.2743 &134 & 123.967\\ ellipticTMR & 3366 & 10920 & 39 & 200 &
         5.15534 &130 & 513.637\\\midrule ex1010 & 0 & 4598 & 25 & 200 &
         5.31938 &176 & 146.81\\ ex1010TMR & 0 & 13804 & 44 & 200 & 5.4995 &134
         & 655.814\\\midrule ex5p & 0 & 1064 & 13 & 200 & 4.95796 &240 &
         30.659\\ ex5pTMR & 0 & 3255 & 22 & 200 & 5.2736 &187 &
         112.023\\\midrule frisc & 886 & 3539 & 23 & 200 & 5.52549 &94.3 &
         144.196\\ friscTMR & 2658 & 10733 & 39 & 200 & 5.82234 &90.1 &
         525.145\\\midrule misex3 & 0 & 1397 & 14 & 200 & 4.93 &244 & 35.739\\
         misex3TMR & 0 & 4205 & 24 & 200 & 5.46202 &198 & 136.961\\\midrule pdc
         & 0 & 4575 & 25 & 200 & 7.21792 &175 & 167.232\\ pdcTMR & 0 & 13765 &
         44 & 200 & 7.51339 &131 & 711.155\\\midrule s298 & 8 & 1930 & 17 & 200
         & 4.76794 &124 & 47.789\\ s298TMR & 24 & 5796 & 29 & 200 & 4.83777
         &116 & 185.169\\\midrule s38417 & 1463 & 6042 & 30 & 200 & 3.53974
         &158 & 241.471\\ s38417TMR & 4389 & 18232 & 51 & 200 & 3.78557 &149 &
         942.7\\\midrule s38584.1 & 1260 & 6177 & 30 & 200 & 3.67662 &206 &
         263.313\\ s38584.1TMR & 3780 & 18835 & 52 & 200 & 4.36338 &157 &
         1199.39\\\midrule seq & 0 & 1750 & 16 & 200 & 5.31333 &253 & 50.624\\
         seqTMR & 0 & 5285 & 27 & 200 & 6.23725 &193 & 184.467\\\midrule spla &
         0 & 3690 & 23 & 200 & 6.44786 &190 & 122.311\\ splaTMR & 0 & 11116 &
         39 & 200 & 6.75377 &152 & 496.416\\\midrule tseng & 385 & 1046 & 13 &
         200 & 3.04212 &161 & 30.561\\ tsengTMR & 1155 & 3260 & 22 & 200 &
         3.82465 &164 & 118.583\\\bottomrule \end{tabularx} \caption{Results
         for 200 width channels} \label{Results200} \end{adjustwidth}
\end{table} \begin{table} \footnotesize \begin{adjustwidth}{-1cm}{-1.2cm}
      \begin{tabularx}{1.1\textwidth}{llllp{1.5cm}p{2.2cm}p{1.5cm}X} \toprule
         Name & Num. Latches & Num. LUTs & FPGA Width & Channel Width & Av. No.
         Wire\newline Segments & Frequency (MHz) & CPU Time (s)\\\midrule alu4
         & 0 & 1522 & 15 & 60 & 4.79098 & 241 & 22.625\\ alu4TMR & 0 & 4574 &
         25 & 60 & 5.38017 & 193 & 101.828\\\midrule apex2 & 0 & 1878 & 16 & 60
         & 5.53926 & 212 & 34.256\\ apex2TMR & 0 & 5637 & 29 & 60 & 6.34397 &
         164 & 153.485\\\midrule apex4 & 0 & 1262 & 14 & 60 & 5.13079 & 241 &
         20.928\\ apex4TMR & 0 & 3805 & 23 & 60 & 5.66967 & 198 &
         90.166\\\midrule bigkey & 224 & 1699 & 16 & 60 & 3.79333 & 421 &
         43.024\\ bigkeyTMR & 672 & 5294 & 27 & 60 & 5.12677 & 363 &
         154.662\\\midrule clma & \multicolumn{7}{c}{Could Not Route}\\ clmaTMR
         & \multicolumn{7}{c}{Could Not Route}\\\midrule des & 0 & 1591 & 16 &
         60 & 4.15635 & 257 & 50.68\\ desTMR & 0 & 5018 & 27 & 60 & 4.07778 &
         230 & 141.366\\\midrule diffeq & 377 & 1494 & 15 & 60 & 3.20978 & 151
         & 28.392\\ diffeqTMR & 1131 & 4521 & 25 & 60 & 3.60614 & 148 &
         115.085\\\midrule dsip & 224 & 1362 & 14 & 60 & 3.84115 & 434 &
         36.32\\ dsipTMR & 672 & 4283 & 24 & 60 & 4.87989 & 368 &
         118.515\\\midrule elliptic & 1122 & 3602 & 23 & 60 & 4.76756 & 137 &
         101.398\\ ellipticTMR & 3366 & 10920 & 39 & 60 & 5.74891 & 133 &
         475.005\\\midrule ex1010 & \multicolumn{7}{c}{Could Not Route}\\
