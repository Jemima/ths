...
\fxfatal{Diagram of tool flow}
\begin{itemize}
   \item Partition
   \item Triplicate
   \item Join
   \item Flatten - Use abc to transform our heirarchical circuit into a format supported by VPR.
   \item Test - Use the verification abilities of abc, described more fully by \fxfatal{reference}. Essentially, given the original circuit and our transformed circuit, verify that every set of inputs produces the same set of outputs in both circuits.
...
The benchmarks used were the 20 largest MCNC LGSynth93 circuits, as provided by the open source VTR project \fxwarning{Reference} and described in table \fxerror{Reference}.
